...
# ╔═╡ d72e8a84-115a-11ec-3173-018f47ffe7ee
begin
...
begin
	...
	n = 500
	argv = Matrix{Float32}(undef, 6, n)
	data = Array{Float32}(undef, 4096, 1, n)
	argv̄ = Matrix{Float32}(undef, 6, n)
	for i in 1:n
		argv[:, i] .= SqState.rand_arg((0, 2), (0, 2π), (0, 0.5))
		state = SqState.construct_state(argv[:, i]..., dim)
		data[:, 1, i] .= rand(state, 4096, IsGaussian)[2, :]
		argv̄[:, i:i] .= m[1:12](data[:, :, i:i])
	end
	
	argv, argv̄
end

# ╔═╡ 21918c27-dfbc-488a-bfd3-adfe0108cbb5
md"
$T \, arg\bar{v} = argv$
"

# ╔═╡ a61d06b2-0e4e-440a-9d7f-4ec49c9f74be
begin
	loader = Flux.DataLoader((argv̄, argv), batchsize=25, shuffle=true)
	
	t = Chain(
		Dense(6, 6),
		x -> vcat(x[1:3, :], x[4:6, :]./sum(x[4:6, :], dims=1))
	)
	arg_data = [(𝐱, 𝐲) for (𝐱, 𝐲) in loader]
...
# ╔═╡ 739e689c-c1e3-4576-86e8-d4e76b99b92d
# t.bias, t.weight, t.σ

# ╔═╡ 634066d9-b9b3-44ab-ab16-60bc3daf0759
i = 3

# ╔═╡ da9df4cb-598d-468f-982a-70c1bffcffed
argv[:, i]

# ╔═╡ 208cd619-8d69-4174-92c6-bf885402a5c5
t(m[1:12](data[:, :, i:i]))
...
SqState = "8d120385-201d-43c1-a82b-e15c39505f50"

[compat]
CUDA = "~3.4.2"
Flux = "~0.12.6"
Plots = "~1.21.3"
QuantumStateBase = "~1.0.2"
SqState = "~0.4.0"
"""
...
git-tree-sha1 = "5a815c6ee86b68b2370b0d7cc5225080003dc3ce"
...
git-tree-sha1 = "a8f2ee4cf9c7c7bb6bac2b1f9a5998821f1e0f41"
...
git-tree-sha1 = "3815eab754a3ca84c20d62e7a7ada51f2c742445"
...
git-tree-sha1 = "560a427826e5ecb674c70c5f09d32e4169c6a1bd"
uuid = "cde9dba0-b1de-11e9-2c62-0bab9446c55c"
version = "0.5.14"

[[Infinities]]
git-tree-sha1 = "48b2fc76ddae59d32c1793e34b158f48cfc794ad"
...
git-tree-sha1 = "5797118cf66028c7c8d4419185ac5668e76d43f8"
...
git-tree-sha1 = "8dfc40073b0110608fd24c112ee99770f81839bd"
...
[[NeuralOperators]]
deps = ["CUDA", "CUDAKernels", "ChainRulesCore", "FFTW", "Flux", "KernelAbstractions", "Tullio", "Zygote"]
git-tree-sha1 = "f1fa4a16d90c7ba60b631d2c5b8b3330ecf33720"
...
git-tree-sha1 = "332a4a8ea799879eab92051bec421b77fc73599b"
...
[[SqState]]
deps = ["CUDA", "DataDeps", "Fetch", "Flux", "JLD2", "NeuralOperators", "QuantumStateBase"]
git-tree-sha1 = "a0039ced8936d6955042e9559aea90d8aa4e34a4"
