...
begin
	wf = WignerFunction(LinRange(-3, 3, 100), LinRange(-3, 3, 100), dim=70)
	...
	function infer_arg(data::Matrix, n_sample::Integer)
	...
	function infer_w(r::T, θ::T, n̄::T, dim::Integer, fix_θ::Bool) where {T<:Real}
		θ = fix_θ ? zero(T) : θ
	...
	function preprocess(data_name::String; n_sample=10, fix_θ=true, dim=70)
		data = get_data(data_name)
		w = infer_w(infer_arg(data, n_sample)..., dim, fix_θ)
...
begin
	data = get_data("SQ20_5mW.mat")
	x_data = sample(data, 4096)
end
...
begin
	anim = @animate for f in readdir(joinpath(datadep"SqState", "data/Flow"))
		plot_wigner(preprocess(f, fix_θ=true), QuantumStatePlots.Contour)
