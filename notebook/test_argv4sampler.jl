...
# ╔═╡ fc601948-0d90-4e28-ad24-29a1b700aed4
begin
...
# ╔═╡ 6a90642c-ff4d-11eb-0c0b-7fd622d3ea7a
md"
# Test arguments
"

# ╔═╡ 735d7391-b7e2-4789-8ed4-cf1168330e18
begin
	state = SqueezedState(ξ(1., 0.))
	θs, xs = LinRange(0, 2π, 100), LinRange(-5, 5, 100)
	pdf = q_pdf(state, θs, xs)
	lim = maximum(abs.(pdf))
	heatmap(θs, xs, pdf', size=(600, 300), color=:coolwarm, clim=(-lim, lim))
end

# ╔═╡ e44eb081-61cc-4348-8956-0c75bcba0b2a
begin
	function clac_σs(state, θs=[0., π/2])
		μs = QSB.π̂ₓ_μ(θs, state)
		σs = real(sqrt.(QSB.π̂ₓ²_μ(θs, state) - μs.^2))
			
		return σs
	end
	

	function plot_σs(r, n̄, dim_range=100:100:1000)
		σs = Matrix{Float64}(undef, 2, length(dim_range))
		for (i, dim) in enumerate(dim_range)
			state = SqueezedThermalState(ξ(r, 0.), n̄, dim=dim)
			σs[:, i] = clac_σs(state) 
		end
			
		p1 = plot(dim_range, σs[1, :], ylabel="sq")
		p2 = plot(dim_range, σs[2, :], ylabel="anti")
		p = plot(p1, p2, layout=@layout([p;p]), size=(600, 300), legend=false)
	end
end

# ╔═╡ 75e60863-696a-457e-b440-3c7bb2763886
md"
## Squeezed state without thermal

**基本上 $r \leq 1.0$ 差異小於小數點下10位以下**

`dim = 100` 夠用
"

# ╔═╡ df2579d9-dd75-4618-a00a-847b3da91479
plot_σs(1., 0.)

# ╔═╡ f7717e69-f3c4-4192-9c55-f66fb1837043
plot_σs(1.1, 0.)

# ╔═╡ 30376d91-b4e3-477b-a37b-439ce38f0e17
plot_σs(1.2, 0.)

# ╔═╡ 548a81db-a80d-4f8b-853b-cb6758f90ade
plot_σs(1.3, 0.)

# ╔═╡ fcc02158-0556-4ae6-a30b-2f52b20c6663
plot_σs(1.4, 0.)

# ╔═╡ 6e803d49-2e78-4610-8fae-9a7da55c9279
plot_σs(1.5, 0.)

# ╔═╡ af42fcac-41eb-48f2-8ebd-cceb402de1e5
plot_σs(2., 0.)

# ╔═╡ bbab5da7-64ea-44da-9da2-35c59b394c73
md"
## Squeezed thermal state
"

# ╔═╡ ed29ba55-2dfe-4219-8d72-22136db7a147
plot_σs(2., 0.)

# ╔═╡ c61a4284-8161-4a8d-90fb-a809270b411e
plot_σs(2., 0.1)

# ╔═╡ 51bca7f7-1410-4950-871c-3455e711dfdc
plot_σs(2., 0.2)

# ╔═╡ 8147b5bf-ee55-40aa-a213-20c97a341450
plot_σs(2., 0.3)

# ╔═╡ f16cbc33-bcf9-4269-ba85-77d292092e0c
plot_σs(2., 0.4)

# ╔═╡ 277f2cc1-637f-4858-beb6-454ff48fd6da
plot_σs(2., 0.5, 800:100:1500)

# ╔═╡ 150d80b1-769f-47be-852d-c574bda3865b
plot_σs(1.5, 0.5)

# ╔═╡ 663b3c55-1f7e-480b-bb55-40f006941a60
plot_σs(1., 0.5)

# ╔═╡ 58b9536f-17b2-415f-9db1-466514dfb587
plot_σs(0., 0.5)
...
git-tree-sha1 = "5bb9f145629b05602fd598fe8698494dc8c4d9b6"
...
git-tree-sha1 = "d198e7abab923d9dce71a4c75d7fea119536665f"
...
git-tree-sha1 = "51d6e30e03c97f95ee0f7c0a3481b7f6eb829d62"
...
git-tree-sha1 = "3d34643d83265e7b03ec3126266b9bd9873a4aea"
...
git-tree-sha1 = "3aeab27ed315e6c5c2895f0e00a01438369750fb"
...
[[QuantumStateBase]]
deps = ["ClassicalOrthogonalPolynomials", "DataDeps", "KernelDensity", "LinearAlgebra", "Mmap"]
git-tree-sha1 = "dd6c7c85e6d60429d4ab6c9e8a75cb60d349edaa"
uuid = "73ce9c4f-35d1-4161-b9e6-26915895bfed"
version = "1.0.1"

[[QuasiArrays]]
deps = ["ArrayLayouts", "DomainSets", "FillArrays", "LazyArrays", "LinearAlgebra", "StaticArrays"]
git-tree-sha1 = "517b4fe8ff413ea8f8401025d391c1745c33d68f"
...
git-tree-sha1 = "a4b7f61b814cdbf5f0729a9e778659001a586760"
