...
function main()
    ...
    w = wf(ρ(VacuumState()))
    file_path = joinpath(SqState.PROJECT_PATH, "../gallery", "wigner_surface_0.png")
    p = plot_wigner(wf, w, Surface, size=(640, 320), file_path=file_path)

    w = wf(ρ(SinglePhotonState()))
    file_path = joinpath(SqState.PROJECT_PATH, "../gallery", "wigner_surface_1.png")
    p = plot_wigner(wf, w, Surface, size=(640, 320), file_path=file_path)
    file_path = joinpath(SqState.PROJECT_PATH, "../gallery", "wigner_contour_1.png")
