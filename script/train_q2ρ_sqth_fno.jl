...
model_name = "q2ρ_sqth_fno"
batch_size = 25
epochs = 30
η₀ = 1e-3

m = SqState.q2ρ_sqth_fno() |> device
...
loader4test = SqState.preprocess_q2ρ(data_files[1], batch_size=batch_size)
...
data_loaders = Channel(5, spawn=true) do ch
    for e in 1:epochs, (i, data_file) in enumerate(data_files[2:end])
        put!(ch, SqState.preprocess_q2ρ(data_file, batch_size=batch_size))
...
for loader4train in data_loaders
    @time begin
        ...
        # descent η
        (t[] > 30) && (opt.os[2].eta = η₀ / 2^ceil((t[]-30)/30))
